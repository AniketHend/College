...
while true; do
	...
	5)
		...
		if [ -f "$db" ]; then
            ...
            read sid
            grep -v $sid $db > temp || echo "record not founds"
            mv temp  $db
            echo "deleted"
		else
			echo "DB not found"
	...
	6)
		...
		if [ -f "$db" ]; then
			...
			if grep -q "^$uid" "$db";then
				echo -e "Enter the field you want to update: \nName\nPhone\nEmail"
				read ch
				case "$ch" in
				Name)
					echo "Enter new name: "
				...
				Phone)
					while true; do
						...
						if [[ "$ph" =~ ^[0-9]{10}$ ]]; then
							...
							echo "Invalid phone number format. Please enter a 10-digit numeric phone number."
						fi
					done
					;;
				Email)
